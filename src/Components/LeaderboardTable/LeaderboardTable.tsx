...
const LeaderboardTable = ({ data }: { data: {}[] | any }) => {
  ...
  return (
    <div className="w-full font-soli flex flex-col bg-primary rounded-md p-3 lg:p-4  flow-hide ">
      {/* inout space */}
      ...
      <input type="text" className="border-none outline-none bg-transparent text-white " value={query} onChange={(e:any) => setQuery(e.target.value)}  />
      </div>
      </div>
      {/* end of input space */}
      ...
      <Pagination data={filteredData} itemsPerPage={10} setCurrentData={setCurrentData} currentData={currentData} />
      </div>
    </div>
  );
};
...
const SingleRow = ({item,index}: {item:any, index:number}) => {
  return (
    <tr className="w-full grid grid-cols-4   text-white place-items-center   px-4 h-10 mt-3 " key={index}>
